...
class Model(nn.Module): 
    def __init__(self, configs):  
        ...
        self.linear4 = nn.Linear(self.hidden_dim, self.hidden_dim)  # 입력 차원에서 숨겨진 차원으로의 선형 변환을 정의합니다.
        self.batch_normalization4 = nn.BatchNorm1d(self.hidden_dim)
        self.relu4 = nn.ReLU()  # ReLU 활성화 함수를 정의합니다.
    ...
    def forward(self, x):  # 순전파 메서드
        ...
        x = self.linear4(x)  # 입력 데이터에 대해 선형 변환을 적용합니다.
        if self.use_batch_norm:
            x = self.batch_normalization4(x)
        x = self.relu4(x)  # ReLU 활성화 함수를 적용하여 비선형성을 추가합니다.
        x = self.dropout4(x) # dropout을 적용하여 일부 node 비활성화
